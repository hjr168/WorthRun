 ...
 * 返回占用高度（含上下间距）。
 */
function drawJudgementBar(ctx, judgement, x, y, w) {
    const barH = 34;
    const padX = 14;
    const gap = 10;
    ...
    const conclusion = (0, format_1.labelOf)(format_1.runJudgementLabels, judgement);
 ...
 */
function drawFitColumns(ctx, suitableFor, notSuitableFor, x, y, w, maxItems = 2) {
    const fit = (suitableFor || []).slice(0, maxItems);
    const avoid = (notSuitableFor || []).slice(0, maxItems);
    if (fit.length === 0 && avoid.length === 0)
        return 0;
    const gap = 8;
    const colW = (w - gap) / 2;
    const padX = 12;
    ...
    const drawColumn = (cx, title, mark, titleColor, items) => {
...
function drawChoiceAndStart(ctx, choiceCounts, startPoint, x, y, w) {
    ...
    if (showChoice) {
        ...
        if (cc.interested > 0)
            parts.push(`已有 ${cc.interested} 人想跑`);
        if (cc.considering > 0)
            parts.push(`${cc.considering} 人在观望`);
        if (parts.length === 0)
            parts.push(`已有 ${cc.total} 人关注`);
    }
...
Page({
    ...
    async drawShareCard(ctx) {
        ...
        const fitRows = Math.max(Math.min((event.suitableFor || []).length, 2), Math.min((event.notSuitableFor || []).length, 2), (event.suitableFor || []).length + (event.notSuitableFor || []).length > 0 ? 1 : 0);
        ...
        const fits = (usedH) => y + usedH <= contentLimit;
        let usedH = (drawTags ? tagBlockH : 0) +
            (drawFit ? fitBlockH : 0) +
        ...
        // 8. 适合谁 / 需要注意（两栏对比）
        if (drawFit) {
            const fitH = drawFitColumns(ctx, event.suitableFor || [], event.notSuitableFor || [], padX, y, contentW, 2);
            if (fitH > 0)
                y += fitH + 12;
        }
        // 9. 关注理由
        ...
        // 10. 跑者关注度 + 起点
        if (drawChoice) {
            const choiceH = drawChoiceAndStart(ctx, cc, hasStart ? event.startPoint : null, padX, y, contentW);
            if (choiceH > 0)
                y += choiceH + 8;
        }
    ...
    drawFooterPanel(ctx, contentBottomY, W, H) {
        const footerPanelH = 104;
    ...
    async drawFooterCode(ctx, rect, event) {
        let codeImage = null;
        try {
