...
- `SHARE_IMAGE_ALLOWED_HOSTS`：后台外部分享图的 HTTPS 主机名白名单；必须与微信小程序 `downloadFile` 合法域名保持一致。
...
首次部署 V0.4.2 时，先预览并初始化来源，再导入年度计划候选：
...
- 赛事详情显示“来源最近检查”；存在开放变更时提示“官方信息近期有更新，正在复核，请前往官方确认。”。
- "前往官方确认"：复制官方链接，并弹窗说明小程序暂不直接跳转外部链接。
- 收藏 / 取消收藏。
- 我的收藏。
- 偏好设置：城市、距离、关注点 chips 多选，支持保存、重置、跳过。
- 赛事纠错与匿名产品反馈；产品反馈只保存文字、页面标识、版本和关联问题编号。
- 我的选择：想跑、观望、已报名，并公开显示匿名聚合数量。
- 原生来源摘要页：展示人工发布的来源摘要和原始来源入口。
- 工具页。
- 配速计算器。
