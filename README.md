...
面向粤港澳大湾区跑者的跑步赛事决策工具。本仓库已完成体验版配置收口、真实赛事 CSV 导入增强、dry-run 校验、路径兼容、后台小程序发布前检查、真实数据内测流程和体验版配置检查。
...
- 小程序内测检查清单和提审准备文档。
- 真实数据内测流程和体验版配置检查清单。
- 小程序体验版配置已指向真实 HTTPS API：`https://run-api.huangjiarong.top`。
...
- `test.ts`：体验版 / 测试环境，当前为 `https://run-api.huangjiarong.top`，已经不是占位域名。
- `prod.ts`：正式环境，当前可暂时与体验版同域名 `https://run-api.huangjiarong.top`；如果后续独立部署正式环境，再替换为独立生产域名。
- `index.ts`：当前启用的环境，体验版阶段指向 `testConfig`。
...
正式发布前如需切换正式环境，修改 `apps/miniapp/config/index.ts`：
...
- 体验版阶段 `apps/miniapp/config/index.ts` 指向 `testConfig`。
- 正式版使用 `prodConfig`；当前可暂时同域，正式环境如后续独立部署再替换。
- 不使用 `localhost`、局域网 IP 或 HTTP 地址提审。
- `test.ts` 已经不是占位域名，不要改回旧的测试占位域名。
- `prod.ts` 当前可暂时同域，不要改回旧的正式占位域名。
- `urlCheck=false` 只允许开发调试，体验版上传和提审前必须开启合法域名校验。
