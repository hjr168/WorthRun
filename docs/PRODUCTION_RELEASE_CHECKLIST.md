...
## 16. V0.5.5 提醒正式上线与运营收口

- [ ] PM2 已改用 `ops/start-api-clean-env.sh`，后台显示运行配置与服务器 `.env` 一致。
...
- [ ] 体验版连续观察 48 小时，无重复发送、模板错误或提醒相关新增 5xx。
- [ ] `--phase=reminders --mode=live` 零阻塞后才切换 `formal`、开启功能并安装 cron。
- [ ] 正式开关开启后 30 分钟内出现任务运行记录，逾期待发送和近 24 小时失败均为 0。
- [ ] 已验证关闭提醒开关并移除 cron 的回退流程，不删除订阅或运行记录。
- [ ] PM2 仍为单个 API 进程，API RSS < 220MB，提醒任务 RSS < 120MB。
